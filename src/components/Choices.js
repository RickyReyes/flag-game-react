    ...
    function handleSelect(e) {
        ...
        let selectedOptionElements = options.map(country => (
            <button disabled
                key={country.area} 
                onClick={(e) => handleSelect(e)}
                className={"self-start text-lg font-bold py-1 px-2 mb-2 border border-indigo-500 rounded-full text-indigo-900 " + (country.name.common == correctCountry.name.common ? "bg-green-300" : e.target.innerText == country.name.common ? "bg-red-300" : "bg-indigo-200")}>
                    {country.name.common}
            </button>
        ))
